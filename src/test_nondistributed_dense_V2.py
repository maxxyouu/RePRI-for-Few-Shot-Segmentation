        ...
        for e in tqdm(range(nb_episodes)):
            ...
            with torch.no_grad():
                for i in range(args.batch_size_val):
                    ...
                    spprt_imgs = spprt_imgs.to(args.device, non_blocking=True)
            ...
            classifier.init_dense_prototypes_v2(features_s, features_q, gt_s, gt_q, classes, callback)
            batch_deltas = classifier.compute_FB_param_dense_v2(features_q=features_q, gt_q=gt_q)
            deltas_init[run, e, :] = batch_deltas.cpu()

            # =========== Perform RePRI inference ===============
            batch_deltas = classifier.dense_v2_RePRI(features_s, features_q, gt_s, gt_q, classes, n_shots, callback)
            deltas_final[run, e, :] = batch_deltas
            t1 = time.time()
            runtime += t1 - t0
            logits = classifier.get_dense_v2_logits(features_q)  # [n_tasks, shot, h, w]
